...
export default async function ShelterPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Posko, Pengungsi & Kebutuhan" description={profile.role === "warehouse_manager" ? "Konteks kebutuhan posko untuk mendukung alokasi gudang dan distribusi bantuan." : "Data dasar untuk menentukan kebutuhan harian, prioritas kelompok rentan, dan alokasi bantuan."} actions={capabilities.canUpdateShelter ? <Dialog><DialogTrigger asChild><Button className="min-h-11"><Plus /> Register Shelter</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Daftarkan posko baru</DialogTitle><DialogDescription>Catat lokasi dan kapasitas awal posko untuk masuk alur kebutuhan.</DialogDescription></DialogHeader><ActionForm action={createShelterAction} className="py-2"><div className="grid gap-2"><Label htmlFor="shelter-name">Nama posko</Label><Input id="shelter-name" name="name" placeholder="Contoh: Posko Balai Nagari" required /></div><div className="grid gap-2"><Label htmlFor="shelter-capacity">Kapasitas</Label><Input id="shelter-capacity" name="capacity" type="number" placeholder="0" min={1} required /></div><SubmitButton>Register Shelter</SubmitButton></ActionForm></DialogContent></Dialog> : null} />
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(20rem,0.65fr)]">
        ...
        {shelterRecommendation ? <RecommendationCard recommendation={shelterRecommendation} /> : null}
      </section>
