...
import { AppSidebar } from "@/app/(console)/_components/sidebar/app-sidebar";
...
export default async function ConsoleLayout({ children }: Readonly<{ children: ReactNode }>) {
  ...
  const [variant, collapsible, profile] = await Promise.all([
    getPreference("sidebar_variant"),
    getPreference("sidebar_collapsible"),
    requireRole(["admin", "bpbd_operator", "field_officer", "shelter_manager", "warehouse_manager"], "/peta-publik"),
  ]);

  const currentUser = profile
    ? {
        name: profile.fullName || roleLabel(profile.role),
        email: profile.email,
